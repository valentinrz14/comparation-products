...
async function explorar(browser, nombre, fn) {
  ...
  await page.screenshot({
    path: `/Users/valentinrodriguez/Downloads/supermarket/debug-${nombre}.png`,
  });
  await ctx.close();
  return result;
}
...
async function main() {
  ...
  await explorar(browser, "COTO-search-url", async (page) => {
    ...
    const url =
      "https://www.cotodigital.com.ar/sitios/cdigi/nuevositio/buscador?query=leche&start=0&sz=10";
    console.log("URL:", url);
    ...
    const precios = await page.$$eval("*", (els) =>
      els
        .filter(
          (el) => el.children.length === 0 && /\$\s*[\d.,]+/.test(el.innerText),
        )
        .slice(0, 10)
        .map((el) => ({
          tag: el.tagName,
    ...
    const prods = await page.$$eval(
      '[class*="product"], [class*="Product"], [class*="item"], article',
      (els) =>
  ...
  await explorar(browser, "COTO-form", async (page) => {
    ...
    const searchInput = await page.$(
      'input[name="query"], input[id*="search"], input[placeholder*="buscar" i], input[placeholder*="Buscar" i]',
    );
    if (searchInput) {
